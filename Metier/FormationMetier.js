class FormationMetier {
    ...
    add() {
        var idF = 0;
       this.Consult();
        if ((document.getElementById('nomF') != null) && (document.getElementById('descF') != null) && (document.getElementById('dateD') != null) && (document.getElementById('nbrM') != null)
            ...
            if (this.isUpdate == true) {
                for (let index = 0; index < ERP.Formation.length; index++) {
                    if (this.formationUpdate.idF == ERP.Formation[index].idF) {
                        this.formation = new Formation(this.formationUpdate.idF, nomF, descF, dateD, nbreM, nomCoash, nomFichier, montant);
        ...
        if (idfUpdate != null) {
            ...
            for (let index = 0; index < ERP.Formation.length; index++) {
                if (idfUpdate == ERP.Formation[index].idF) {
                    localStorage.setItem("Update", JSON.stringify(ERP.Formation[index]))
    ...
    fill() {
        if (localStorage.getItem("Update") != null) {
            this.formationUpdate = JSON.parse(localStorage.getItem("Update"));

            document.getElementById('nomF').value = this.formationUpdate.nomF;
            document.getElementById('descF').value = this.formationUpdate.descF;
            document.getElementById('dateD').value = this.formationUpdate.dateD;
            document.getElementById('nbrM').value = Number(this.formationUpdate.nbreM);
            document.getElementById('nomCoash').value = this.formationUpdate.nomCoash;
            //document.getElementById('nomFichier').value = this.formatiomUpdate.nomFichier;
            document.getElementById('montant').value = this.formationUpdate.montant;
